...
class DiscriminativeModelCalibration(Task):
    ...
    def __init__(self,model):
        ...
        self.logPosteriorsTraining = torch.tensor(expit(modelOutputs["train"]["logits"]),dtype=torch.float32)
        self.logPosteriorsValidation = torch.tensor(expit(modelOutputs["validation"]["logits"]),dtype=torch.float32)
